...
function consolidateCompsAndFootage() {
    ...
    var consolidateFootageOption = optionsGroup.add("checkbox", undefined, "Consolidate Footage");
    var consolidateCompsOption = optionsGroup.add("checkbox", undefined, "Consolidate Compositions");
    var consolidateNullsOption = optionsGroup.add("checkbox", undefined, "Consolidate Nulls & Solids");
...
function consolidateCompositions() {
    ...
    for (var i = 0; i < allComps.length; i++) {
        ...
        var key = compA.name + "|" + compA.width + "|" + compA.height + "|" + compA.frameRate + "|" + compA.duration;
        if (consolidated[key]) {
...
function consolidateSolidsAndNulls() {
            ...
            for (var j = 1; j <= item.numLayers; j++) {
                ...
                if (layer instanceof AVLayer) {
                    ...
                    if (layer.source.mainSource instanceof SolidSource) {
                        var solidKey = "Solid|" + layer.source.mainSource.color.toString() + "|" + layer.source.width + "|" + layer.source.height + "|" + layer.source.pixelAspect;
    ...
    for(var i = app.project.numItems; i >= 1; i--){
        ...
        if(app.project.item(i).mainSource instanceof SolidSource && app.project.item(i).usedIn.length == 0){
